...
public abstract class GameEngine2D {
  ...
  // These keys are handled differently by the game engine in that they can be
  // held down
  ...
  // tracks if the game is currently paused or not (i.e. the player pressed the
  // 'P' key).
  ...
  // First element is drawn underneath all entities, last element ontop of all
  // entities.
  ...
  // if set to true: the game automatically pauses when a splash screen is
  // displayed
  ...
  private void gameLoop() {
    ...
    while (!isGameOver()) {
      ...
      // Garbage Collects any Entities that are flagged for removal from the game
      // window
      entities.performGC();
  ...
  private void captureInput() {
    ...
    // The player could be pressing multiple keys simultaneously, ex: moving
    // diagonally.
  ...
  // window.
  // This method returns a Collection containing any Entities that are colliding
  // with
  // the argument Entity
  ...
  // Returns the number of iterations of the game loop since the game has been
  // runninng.
